...
class PortfolioList implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
        if(function_exists('vc_map')) {
            ...
            if(elated_cpt_theme_installed()) {
                $icons_array = $moose_elated_icon_collections->getVCParamsArray(array('element' => "show_load_more", 'value' => array('yes','')));
            }
    ...
    public function render($atts, $content = null) {
        ...
        if (isset($moose_elated_options['portfolio_eltd_like'])) {
            $portfolio_eltd_like = $moose_elated_options['portfolio_eltd_like'];
        }

        $portfolio_filter_class = "";
        if (isset($moose_elated_options['portfolio_filter_disable_separator']) && !empty($moose_elated_options['portfolio_filter_disable_separator'])) {
            if ($moose_elated_options['portfolio_filter_disable_separator'] == "yes") {
        ...
        if (isset($moose_elated_options['paspartu_portfolio_full_width']) && $moose_elated_options['paspartu_portfolio_full_width'] =='yes' ){
        ...
        if ($overlay_background_color != '' || $gradient_position != '') {
            if ($hover_type == "gradient_hover") {
                if (substr($overlay_background_color, 0, 3) === "rgba") { // if rgba is set, portfolio uses default color
                    ...
                    $rgb = moose_elated_hex2rgb($overlay_background_color);
        ...
        if ($type != 'masonry') {
            ...
            if (have_posts()) : while (have_posts()) : the_post();
                    ...
                    if ($overlay_background_color_custom != '' || $gradient_position != '') {
                        if ($hover_type == "gradient_hover") {
                            if (substr($overlay_background_color_custom, 0, 3) === "rgba") { // if rgba is set, portfolio uses default color
                                ...
                                $rgb = moose_elated_hex2rgb($overlay_background_color_custom);
                            ...
                            if ($show_icons == 'yes') {
                                ...
                                if ($portfolio_eltd_like == "on" && $show_like == "yes") {
                                    if (function_exists('elated_cpt_like_portfolio_list')) {
                                        $html .= elated_cpt_like_portfolio_list(get_the_ID());
                                    }
                                }
                                if ($link_icon == "yes") {
                ...
                if ($type == "standard" || $type == "standard_no_space" || $type == "masonry_with_space") {
                    $html .= "<div class='portfolio_description " . $portfolio_description_class . "' ".moose_elated_get_inline_style(implode('; ', $portfolio_box_style)).">";
            ...
            if (get_next_posts_link()) {
                if ($show_load_more != 'no') {
                    $html .= '<div class="portfolio_paging" ' . moose_elated_get_inline_style($show_more_style) . '>
					<span data-rel="' . $wp_query->max_num_pages . '" class="load_more '. $animation_class . $icon_class . ' " '. moose_elated_get_inline_style($show_more_inner_style) . ' >' . get_next_posts_link( $span_text_holder_open . $span_overlay . $span .  __('Show more', 'eltd_core') . $span_close .  $span_hidden_text .  __('Show more', 'eltd_core') . $span_close . $span_close . $span_icon_holder_open . $span_hidden_icon . $add_icon . $span_close . $span . $add_icon . $span_close . $span_close . $span ) . '</span></div>';
					
					
					
					
					
                    $html .= '<div class="portfolio_paging_loading" ' . moose_elated_get_inline_style($show_more_style) . '><a href="javascript: void(0)" class="qbutton" ' . moose_elated_get_inline_style($show_more_inner_style) . '><i class="fa fa-spinner fa-spin"></i></a></div>';
                }
            }
