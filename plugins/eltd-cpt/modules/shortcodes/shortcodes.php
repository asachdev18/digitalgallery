<?php
if (!function_exists('eltd_cpt_register_button')) {

    function eltd_cpt_register_button($buttons) {
        array_push($buttons, "|", "eltd_shortcodes");
        return $buttons;
    }
}

if (!function_exists('eltd_cpt_add_plugin')) {

    function eltd_cpt_add_plugin($plugin_array) {
...
if (!function_exists('moose_elated_shortcodes_button')) {

    function moose_elated_shortcodes_button() {
        if (!current_user_can('edit_posts') && !current_user_can('edit_pages')) {
            return;
        }

        if (get_user_option('rich_editing') == 'true') {
            add_filter('mce_external_plugins', 'eltd_cpt_add_plugin');
            add_filter('mce_buttons', 'eltd_cpt_register_button');
        }
    }

}
add_action('after_setup_theme', 'moose_elated_shortcodes_button');
...
if (!function_exists('no_call_to_action')) {
    ...
    function no_call_to_action($atts, $content = null) {
        ...
        global $moose_elated_icon_collections, $moose_elated_options;
        ...
        $args = array_merge($args, $moose_elated_icon_collections->getShortcodeParams(),$call_to_action_icons_form_fields);
        ...
        if ($border_color != '') {
            $action_styles .= 'border: 2px solid ' . $border_color . ';';
			
        }
		
		$button_icon_border_styles="";
		if ($button_border_color != "") {
            $button_icon_border_styles .= 'border-left-color: ' . $button_border_color . '; ';
        }
		...
		if ($show_button == 'yes') {
			...
			if($button_hover_animation == "rotate_icon" || $button_hover_animation == "rotate_text_and_icon" || $button_hover_animation == "fill_text_rotate_icon"){
				$animate_button = true;
			}
			elseif($button_hover_animation == "disable_animation"){
				$animate_button = false;
			}
			if($button_hover_animation == "default" && isset($moose_elated_options['button_hover_animation']) && ($moose_elated_options['button_hover_animation'] !== '')){
				$animate_button = true;
			}
			elseif($button_hover_animation == "default" && isset($moose_elated_options['button_hover_animation']) && ($moose_elated_options['button_hover_animation'] == '') ){
				$animate_button = false;
			}
			...
			if ($animate_button == true){
				...
				elseif(isset($moose_elated_options['button_hover_animation']) && ($moose_elated_options['button_hover_animation'] != '')){
        ...
        $html.= '<div class="call_to_action ' . esc_attr($action_classes) . '" '.moose_elated_get_inline_style($action_styles).'>';
        ...
        if ($show_button == 'yes') {
            if ($grid_size == "75") {
                $html .= '<div class="call_to_action_row_75_25 clearfix" '.moose_elated_get_inline_style($action_inner_styles).'>';
            }
            elseif ($grid_size == "66") {
                $html .= '<div class="call_to_action_row_66_33 clearfix" '.moose_elated_get_inline_style($action_inner_styles).'>';
            }
            else {
                $html .= '<div class="call_to_action_row_50_50 clearfix" '.moose_elated_get_inline_style($action_inner_styles).'>';
            }
        ...
        if ($type == "with_icon" || $type == "with_custom_icon") {
            ...
            $html .= '<div class="call_to_action_icon_inner" '.moose_elated_get_inline_style($icon_inner_style).'>';
            if ($custom_icon != "") {
                ...
                $icon_collection_obj = $moose_elated_icon_collections->getIconCollection($icon_pack);
        ...
        $html .= '<div class="call_to_action_text'. esc_attr($call_to_action_text_class) .'" '.moose_elated_get_inline_style($content_styles).'>'.do_shortcode($content).'</div>';
        ...
        if ($show_button == 'yes') {
            ...
            if ($button_icon_pack != ''){
                $icon_collection_obj = $moose_elated_icon_collections->getIconCollection($button_icon_pack);
			...
			$html .= '<a href="' . $button_link . '" target="' . $button_target . '" ' . $data_attr . ' class="' . $button_classes . '" style="' . $button_styles . '">
		<span class="text_holder"><span class="a_overlay"></span><span>' . $button_text .'</span><span  class="hidden_text">' . $button_text  .'</span></span>
		<span class="icon_holder" style="' . $button_icon_styles . $button_icon_border_styles . '"><span>'. $button_icon .'</span><span class="hidden_icon">' . $button_icon . '</span></span></a>';
           // $html .= '<a href="' . esc_url($button_link) . '" class="qbutton ' . esc_attr($button_size) . '" '.eltd_get_inline_attr($button_target, 'target').' '.eltd_get_inline_style($button_styles).' ' . $data_attr . '><span class="call_to_action_button_text">' . wp_kses_post($button_text) .'</span>'. $button_icon . '</a>';
...
if (!function_exists('no_blockquote')) {
    ...
    function no_blockquote($atts, $content = null) {
        ...
        //line height for this icon should be smaller than font size

        if($moose_elated_options['blockquote_icon_type'] == "" || $moose_elated_options['blockquote_icon_type'] == "icon_quotations"){
            $blockquote_classes[] = 'with_icon_font';
        }

        $html .= "<blockquote class='" . implode(' ', $blockquote_classes) . "' style='" . $blockquote_styles . "'>"; //open blockquote
        if ($show_quote_icon == 'yes') {
            if($quote_icon_font == 'with_icon'){
				if(isset($moose_elated_options['blockquote_icon_type'])) { //if icon is set in options
					$html .= '<span style="' . implode(";", $quote_icon_styles) . '" class="icon_quotations_holder">';
					if($moose_elated_options['blockquote_icon_type'] == '')  { //if selected icon is empty use default icon
						$html .= '<i class="eltd_icon_font_elegant icon_quotations "></i>';
					}
					else {
						$html .= '<i class="' . $moose_elated_options['blockquote_icon_type'] . '"></i>';
					}					
					$html .= '</span>';
				} 
            }
...
if (!function_exists('no_button')) {
    ...
    function no_button($atts, $content = null) {
        ...
        $args = array_merge($args, $moose_elated_icon_collections->getShortcodeParams());
		...
		if($button_hover_animation == "fill_text_rotate_icon"){
			$fill_overlay = true;
		}
		
		elseif($button_hover_animation == "default" && isset($moose_elated_options['button_hover_animation']) && ($moose_elated_options['button_hover_animation'] == 'fill_text_rotate_icon')){
			$fill_overlay = true;
		}
		...
		$button_icon_styles="";
		if ($border_color != "") {
            $button_icon_styles .= 'border-left-color: ' . $border_color . '; ';
        }

        if ($border_width != "" && $style !== "transparent") {
            $border_width = (strstr($border_width, 'px', true)) ? $border_width : $border_width . "px";
            $button_styles .= 'border-width: ' . esc_attr($border_width) . '; ';
            $button_icon_styles  .= 'border-left-width: ' . esc_attr($border_width) . '; ';
        }
        ...
        if ($icon_pack != "") {
            ...
            if ($icon_color != "" && $animate_button == false){
        ...
        if ($margin != "") {
		 ...
		 if ($style !== "transparent"){
	        ...
	        if ($hover_background_color != "" && $fill_overlay == false) {
	            $data_attr .= "data-hover-background-color=" . $hover_background_color . " ";
	        }
			
			
			if ($hover_background_color != "" && $fill_overlay == true){
        ...
        if ($hover_color != "" && $animate_button == false) {
            $data_attr .= "data-hover-color=" . $hover_color . " ";
        }
		
		
		$animation_color_hover = '';
		if ($hover_color != "" && $animate_button == true && $fill_overlay == false){
			$animation_color_hover .= 'color: ' . $hover_color . '; '; 
        }
		
		$animation_fill_color_hover = '';
		if ($hover_color != "" && $animate_button == true && $fill_overlay == true){
			$animation_fill_color_hover .= 'color: ' . $hover_color . '; '; 
        }
		

		
		$html .= '<a href="' . $link . '" target="' . $target . '" ' . $data_attr . ' class="' . $button_classes . '" style="' . $button_styles . '">
		<span class="text_holder"><span class="a_overlay" style="' . $animation_overlay_styles . '"></span><span>' . $text .'</span><span style="' . $animation_color_hover . '" class="hidden_text">' . $text  .'</span></span>
		<span class="icon_holder" style="' . $button_icon_styles . '"><span  style="' . $animation_fill_color_hover . '">'. $add_icon .'</span><span style="' . $animation_color_hover . '" class="hidden_icon">' . $add_icon . '</span></span></a>';
...
if (!function_exists('no_counter')) {
    ...
    function no_counter($atts, $content = null) {
		...
		if ($separator == "yes" && $separator_position == "above_title" ) {
            $html .= '<span class="separator medium" ' . moose_elated_get_inline_style($separator_styles) . '></span>';
        }
...
//		innner div style
        ...
        $html .= '<div class="custom_font_holder" '.  moose_elated_get_inline_style($custom_font_wrapper_styles).' '.$data_attr.'>';

        //set data attrs for noTypeFont function

        $type_data .= "data-loop=" . $loop . " ";
        $type_data .= "data-speed=" . $type_speed . " ";
        $type_data .= "data-back_delay=" . $back_delay . " ";

        //see whether to type out strings or show plain text

        if ($type_out_strings == 'no') {

		    $html .= '<div '.$custom_font_inner_class.' '.moose_elated_get_inline_style($custom_font_inner_style).'>'.do_shortcode($content).'</div>';

        }

        elseif (($type_out_strings == 'yes') ) {

            $html .= '<span class="typed" '.$type_data. ' ' .$custom_font_inner_class.' '.moose_elated_get_inline_style($custom_font_inner_style).'>';
            if ($first_string !== '') { 
            
                $html .= '<span>'.esc_attr($first_string).'</span>';
            
            } if ($second_string !== '') {

                $html .= '<span>'.esc_attr($second_string).'</span>';

            } if ($third_string !== '') {

                $html .= '<span>'.esc_attr($third_string).'</span>';
...
if (!function_exists('no_cover_boxes')) {
    ...
    function no_cover_boxes($atts, $content = null) {
        ...
        $html .= "<div class='cover_boxes ".esc_attr($items_number_class)."' data-active-element='" . $active_element . "' ".moose_elated_get_inline_attr($items_number,"date-number_of_items")."><ul class='clearfix'>";
        ...
        $html .= '<div class="box_content_inner">';
        ...
        if ($link_label1 != "") {
            ...
            if ($cover_boxes_icon_pack != ''){
                $icon_collection_obj = $moose_elated_icon_collections->getIconCollection($cover_boxes_icon_pack);
...
if (!function_exists('no_icons')) {
    ...
    function no_icons($atts, $content = null) {
        ...
        $default_atts = array(
            ...
            "icon_hover_effect" => "no_effect",
			"outline_color" => ""
        );

        $default_atts = array_merge($default_atts, $moose_elated_icon_collections->getShortcodeParams());
        ...
        $icon_style = '';               //icon
        $hover_icon_style = '' ; //hover icon for roll over
        //generate data attr
        ...
        $roll_over = '';
        $roll_over_element_background_color = '';
        $roll_over_element_border_color = '';
        ...
        $fa_size = $moose_elated_icon_collections->getIconSizeClass($fa_size);
        ...
        if ($icon_hover_animation == 'roll_over' || $icon_hover_effect == 'roll_over') {
             $icon_style .= 'color:' . $icon_color . ';'; 
             $hover_icon_style .= 'color:' . $hover_icon_color . '!important;';
        }
        ...
        if ($hover_border_color != "") {
            ...
            $roll_over_element_border_color .= 'border-color:' . $hover_border_color . ";";
        }

        if ($hover_background_color != "") {
            $data_attr_stack .= "data-hover-background-color=" . $hover_background_color . " ";
            $roll_over_element_background_color .= 'background-color:' . $hover_background_color . ";" ;
        }
        ...
        if ($type !='normal' && $icon_hover_animation == 'roll_over') { 

            $html .= '<div class ="roll_over_icon_wrapper">'; //open circle or square icon wrapper when roll over set

        }

        $html .= '<span class="eltd_icon_shade eltd_icon_shortcode ' . esc_attr($roll_over) . esc_attr($icon_pack . ' ' . $type . ' ' . $icon_stack_classes . ' ' . $hover_animation_class . ' ' . $icon_animation) . '" ' . $data_attr_stack . ' '.moose_elated_get_inline_style($icon_stack_style.$animation_delay_style).'>';
        if ($link != "") {
            $html .= '<a href="' . esc_url($link) . '" target="' . esc_attr($target) . '" '.moose_elated_get_inline_style($icon_link_style).' '. moose_elated_get_class_attribute($icon_link_classes) . '>';
        }
        ...
        if (method_exists($icon_collection_obj, 'render')) {
            ...
            if ($type == 'normal' && $icon_hover_effect == "roll_over") {
                $html .= $icon_collection_obj->render(${$icon_collection_obj->param}, array(
                    'icon_attributes' => array_merge(
                            array(
                        'class' => 'hover_icon',
                        'style' => $icon_style . ' ' . $hover_icon_style 
                        ), $data_attr_icon
                    )
                ));
            }
		...
		if (($type == 'square' || $type == 'circle') && $icon_hover_animation != 'no_animation'){
			$html .= '<span class="animation_overlay" '.moose_elated_get_inline_style($animation_icon_style).'></span>';
		}
        ...
        if ($type !='normal' && $icon_hover_animation == 'roll_over') {
                    ...
                    $html .= '<span class="eltd_icon_shade eltd_icon_shortcode ' . esc_attr($roll_over) . esc_attr($icon_pack . ' ' . $type . ' ' . $icon_stack_classes . ' ' . $hover_animation_class . ' ' . $icon_animation) . '" ' . $data_attr_stack . ' '.moose_elated_get_inline_style($icon_stack_style.$animation_delay_style.$roll_over_element_background_color.$roll_over_element_border_color).'>';
                    if ($link != "") {
                        $html .= '<a href="' . esc_url($link) . '" target="' . esc_attr($target) . '" '.moose_elated_get_inline_style($icon_link_style).' '. moose_elated_get_class_attribute($icon_link_classes) . '>';
                    }
                    ...
                    if (method_exists($icon_collection_obj, 'render')) {
                        $html .= $icon_collection_obj->render(${$icon_collection_obj->param}, array(
                            'icon_attributes' => array_merge(
                                    array(
                                'class' => 'hover_icon',
                                'style' => $icon_style . ' ' . $hover_icon_style . ' ' . $animation_delay_style
                                ), $data_attr_icon
                            )
                        ));
                    }
                    ...
                    if (($type == 'square' || $type == 'circle') && $icon_hover_animation != 'no_animation'){
                        $html .= '<span class="animation_overlay" '.moose_elated_get_inline_style($animation_icon_style.$roll_over_element_background_color).'></span>';
                    }
                    ...
                    $html .= '</div>'; // close circle or square icon wrapper when roll over set
        }
...
if (!function_exists('no_icon_text')) {
    ...
    function no_icon_text($atts, $content = null) {
        ...
        $title = esc_html($title);
        $title_font_size = esc_html($title_font_size);
        $title_color = esc_attr($title_color);
		...
		$icon_animation_hover = esc_attr($icon_animation_hover);
		$outline_color = esc_attr($outline_color);
		$outline_hover_color = esc_attr($outline_hover_color);

        $icon_size = $moose_elated_icon_collections->getIconSizeClass($icon_size);
        ...
        if ($icon_color != "") {
		 ...
		 if ($outline_color != "") {
            $animation_outline_color .= 'box-shadow: 0 0 0 2px ' . $outline_color . ';';
        }
        ...
        if ($icon_pack == 'font_awesome' && !empty($fa_icon)) {
            ...
            switch ($icon_size) {
                case 'eltd_tiny_icon': //smallest icon size
                    $box_size = 'tiny';
                    break;
                case 'eltd_small_icon':
                    $box_size = 'small';
                    break;
                case 'eltd_medium_icon':
                    $box_size = 'medium';
                    break;
                case 'eltd_large_icon':
                    $box_size = 'large';
                    break;
                case 'eltd_huge_icon':
                    $box_size = 'very_large';
        ...
        if (($custom_icon !== "") && is_numeric($custom_icon)) {
			$icon_with_text_clasess	.= 'custom_icon_text ';
            $custom_icon_src = wp_get_attachment_url($custom_icon);
		...
		$outline_data="";
		if ($outline_hover_color !== "") {
            $outline_data .= 'data-outline-hover-color=' . $outline_hover_color . ' ';
        }
		if ($outline_color !== "") {
			$outline_data .= 'data-outline-color=' . $outline_color . ' ';
        }
            ...
            case 'circle':
                ...
                if (method_exists($icon_collection_obj, 'render')) {
                    ...
                    if ($icon_pack == 'font_elegant' && !empty($fe_icon)) {
                        ...
                        $html_icon .= '<span ' . $outline_data . ' class="animation_overlay" '.moose_elated_get_inline_style($animation_outline_color).' ></span></span>';
                    } else {
        ...
        if ($box_type == "normal") {
			...
			if ($icon_animation_hover == "outline_enlarge") {
                $icon_with_text_clasess .= " outline_enlarge";
            }
            if ($icon_animation_hover == "roll_over") {
                $icon_with_text_clasess .= " roll_over";
            }
...
//            OPEN SHORTCODE HTML
            $html .= "<div class='eltd_icon_with_title " . $icon_with_text_clasess . "' " . esc_attr($icon_data) . ">";
...
//          ICON POSITION LEFT FROM TITLE
            elseif ($icon_position == 'left_from_title') {
                ...
                // If icon is image
                if ($custom_icon !== "") {
                    $html .= '<div class="' . $custom_icon_holder . ' ' . $icon_animation . $custom_icon_holder .  '" style="' . $icon_margin_style . ' ' . $animation_delay_style . '">';
                    $html .= $custom_icon_html;
                    $html .= '</div>'; //close icon_holder
                } else {
...
//            ICON POSITION LEFT
            ...
            $html .= '<div class="eltd_box_holder with_icon' . esc_attr($box_holder_classes) . '" style="' . $box_holder_styles . '" ' . esc_attr($icon_data) . '>';
...
if (!function_exists('no_image_hover')) {
    ...
    function no_image_hover($atts, $content = null) {
        ...
        if ($animation_speed != "") {
            $transition_property = "all " . $animation_speed . "s ease-in-out";
        ...
        if ($hover_effect == "roll_over") {
            $animate_class .= " roll_over";
        }
...
if (!function_exists('no_image_carousel')) {

    function no_image_carousel($atts, $content = null) {
        $args = array(
            "images" => "",
            "on_click" => "",
            ...
            "separator_margin_top" => "18",
            "separator_margin_bottom" => "12",
            "separator_color" => "#e5735c", 
        ...
        $eltd_image_carousel_separator_style = '';

        $image_carousel_title_style = '';
        $image_carousel_content_style = '';

        if ($title_color != '') {
            $image_carousel_title_style = ' color: ' . $title_color;
        }
        
        if ($content_color != '') {
            $image_carousel_content_style = ' color: ' . $content_color;
        }

        //separator styling
        if ($separator_width != '') {
            $eltd_image_carousel_separator_style .= ' width: ' . $separator_width . 'px;';
        }
        if ($separator_thickness != '') {
            $eltd_image_carousel_separator_style .= ' border-bottom-width: ' . $separator_thickness . 'px;';
        }
        if ($separator_margin_top != '') {
            $eltd_image_carousel_separator_style .= ' margin-top: ' . $separator_margin_top . 'px;';
        }
        if ($separator_margin_bottom != '') {
            $eltd_image_carousel_separator_style .= ' margin-bottom: ' . $separator_margin_bottom . 'px;';
        }
        if ($separator_color != '') {
            $eltd_image_carousel_separator_style .= ' border-color: ' . $separator_color . ';';
        }
        ...
        $html .="<div class='no_image_carousel'>"; // image carousel holder

        $html .="<div class='no_image_carousel_inner' style='background-color:".$background_color.";' >"; 

        $html .="<div class='no_image_carousel_info_holder'>";  // content + navigation holder

        $html .= "<div class='no_image_carousel_decription_holder'>"; //content 

        if ($image_carousel_title != '') {

            $html .= "<div class='no_image_carousel_title'>";

            $html .= "<h3 style='" . $image_carousel_title_style . "'>".esc_attr($image_carousel_title)."</h3>";

            $html .= "</div>";
        }

        if ($image_carousel_separator != '') {

            $html .= "<div class='no_image_carousel_separator'>";

            $html .= "<span class='separator small' style='" . $eltd_image_carousel_separator_style . " display:inline-block;'></span>";

            $html .= "</div>";

        }

        if ($image_carousel_content != '') {

            $html .= "<div class='no_image_carousel_content'>";

            $html .= "<p style='" . $image_carousel_content_style . "'>".esc_attr($image_carousel_content)."</p>";

            $html .= "</div>";
        }

        $html .="</div>"; // end content holder

        $html .= '<div class="controls no_image_carousel_navigation">'; //navigation
        ...
        $html .= "</div>"; // end  content + navigation holder

        $html .= "<div class='no_image_carousel_slides_holder'>"; // image carousel slides holder

        $html .="<ul class='slides'>"; // slides list

        if (isset($images_gallery_array) && count($images_gallery_array) != 0) {

            $i = 0;

            foreach ($images_gallery_array as $img_id) {

                $image_src = wp_get_attachment_image_src($img_id, 'full', true);
                $single_image_alt = get_post_meta($img_id, '_wp_attachment_image_alt', true);
                $single_image_title = get_the_title($img_id);
                $single_image_description = get_post($img_id)->post_content;

                $single_image_src = $image_src[0];
                $single_image_width = $image_src[1];
                $single_image_height = $image_src[2];

                $html .= '<li class="slide">';

                if ($on_click == 'prettyPhoto') {

                    $html .= '<a class="prettyPhotolink" data-rel="' . esc_attr($pretty_photo_rel) . '" href="' . esc_url($single_image_src) . '" title="'.esc_attr($single_image_description).'">'; //prettyPhotolink

                    $html .= '</a>';

                    $html .= '<span class="shader"></span>';

                }

                $html .= '<img src="' . esc_url($single_image_src) . '" alt="' . esc_attr($single_image_alt) . '" />'; //single image output
        ...
        $html .="</ul>"; //end slides list

        $html .="</div>"; //end image carousel slides holder

        $html .="</div>"; //end image carousel inner holder

        $html .="</div>"; //end image carousel holder

        return $html;

    }

    add_shortcode('no_image_carousel', 'no_image_carousel');
...
if (!function_exists('no_icon_list_item')) {
    ...
    function no_icon_list_item($atts, $content = null) {
        ...
        $icon_margin_left = esc_attr($icon_margin_left);
        $border_color = esc_attr($border_color);
        $title = esc_html($title);
        $title_color = esc_attr($title_color);
        $title_size = esc_attr($title_size);
        ...
        if ($icon_position == 'left' && $icon_margin_right !== "") {
		...
		if ($icon_position == 'right' && $icon_margin_left !== "") {
            $icon_margin_left = (strstr($icon_margin_left, 'px', true)) ? $icon_margin_left : $icon_margin_left . 'px';
            $icon_style .= " margin-left: " . $icon_margin_left . ";";
        }
        ...
        $html .= '<div class="eltd_icon_list '.$icon_position.'" '.$item_margin_style .'>';

		if($icon_position == 'right') {
			$html .= '<p class="eltd_icon_list_text" '.moose_elated_get_inline_style($title_style) . '>' . $title . '</p>';
		}
        ...
        if (method_exists($icon_collection_obj, 'render')) {
            $html .= $icon_collection_obj->render(${$icon_collection_obj->param}, array(
                'icon_attributes' => array(
                    'style' => $icon_style,
                    'class' => $icon_classes . ' ' . $border_type .' eltd_icon_list_item_icon'
                )
            ));
        }
...
if (!function_exists('no_image_with_text_and_icon')) {
    ...
    function no_image_with_text_and_icon($atts, $content = null) {
        ...
        foreach ($moose_elated_icon_collections->iconCollections as $icon_set) {
            if (${$icon_set->param}) {
...
if (!function_exists('no_blog_list')) {
    ...
    function no_blog_list($atts, $content = null) {
        ...
        if ($image_size !== '' && $image_size == "landscape") {
            $thumb_image_size .= 'eltd-moose-portfolio-landscape';
        } else if($image_size === 'square'){
			$thumb_image_size .= 'eltd-moose-portfolio-square';
        ...
        while ($q->have_posts()) : $q->the_post();
            ...
            switch ($type) {
				case 'image_in_box':
                        ...
                        $post_info_html .= '<div class="post_info_section" ' . moose_elated_get_inline_style($latest_post_info_style_array) . '>';

                        if ($display_date == '1' && $date_place == 'by_post_info') {
                            $post_info_html .= '<span class="date" ' . moose_elated_get_inline_style($date_style) . '>' . get_the_time('j F Y') . ' </span>'; //close date_hour_holder
                        }
                        ...
                        if ($display_comments == "1") {
                          $comments_count = get_comments_number();
                            ...
                            switch ($comments_count) {
                                case 0:
                                    $comments_count_text = esc_html__('No comment', 'moose');
                                    break;
                                case 1:
                                    $comments_count_text = $comments_count . ' ' . esc_html__('Comment', 'moose');
                                    break;
                                default:
                                    $comments_count_text = $comments_count . ' ' . esc_html__('Comments', 'moose');
                                    break;
                            }
                            $post_info_html .= '<div class="latest_post_comments"> ';
                            $post_info_html .= '<a '.moose_elated_get_inline_style($latest_post_info_link_color).' class="post_comments" href="' . esc_url(get_comments_link()) . '">';
                        ...
                        if ($display_category == '1') {
                            ...
                            $post_info_html .= esc_html__('in ','moose');

                            foreach ($cat as $categ) {
                                $post_info_html .= '<a '.moose_elated_get_inline_style($latest_post_info_link_color).' href="' . esc_url(get_category_link($categ->term_id)) . '">' . esc_html($categ->cat_name) . ' </a> ';
                            }
                        ...
                        if ($display_author == '1') {
                            ...
                            $post_info_html .= '<span '.moose_elated_get_inline_style($latest_post_info_color).'>' . esc_html__("by", 'moose') . '</span> <a class="post_author_link" href="' . esc_url(get_author_posts_url(get_the_author_meta("ID"))) . '"><span '.moose_elated_get_inline_style($latest_post_info_link_color).' >' . esc_html(get_the_author_meta("display_name")) . '</span></a>';
                            $post_info_html .= '</div>'; //close span.latest_post_categories
                        }
                        ...
                        $html .= '<div class="latest_post" ' . moose_elated_get_inline_style($box_info_style) . '>';
                        ...
                        if ($display_date == '1' && $date_place == 'over_title') {
                            ...
                            $html .= '<span class="date" ' . moose_elated_get_inline_style($date_style) . '>' . get_the_time('j F Y') . ' </span>'; //close date_hour_holder
                            $html .= '</div>';
                        }

                        $html .= '<div class="latest_post_title_holder">';

                        $html .= '<' . $title_tag . ' class="latest_post_title " ' . moose_elated_get_inline_style($title_style) . '>';
                        if ($display_date == '1' && $date_place == 'by_title') {
                            $html .= '<span class="date" ' . moose_elated_get_inline_style($date_style) . '>' . get_the_time('d M') . ' </span>'; //close date_hour_holder
                        }
                        ...
                        if ($display_excerpt == '1' && $text_length != '0') {
                            ...
                            $html .= '<p class="excerpt" ' . moose_elated_get_inline_style($excerpt_style) . '>' . $excerpt . '...</p>';
                        }
				...
				case 'boxes':
                        ...
                        $icon_color = '';
                        if($post_info_color != ""){
                            $icon_color .= 'color: '.$post_info_color.';';
                        }
                        ...
                        if ($display_author == '1') {
                            $post_info_html .= '<div class="latest_post_author">';
                            $post_info_html .= '<a class="post_author_link" href="' . esc_url(get_author_posts_url(get_the_author_meta("ID"))) . '">';
							$post_info_html .= '<span class = "author_icon icon-user" '.moose_elated_get_inline_style($icon_color).'></span>';
							$post_info_html .= '<span '.moose_elated_get_inline_style($latest_post_info_link_color).' >' . esc_html(get_the_author_meta("display_name")) . '</span>';
							$post_info_html .= '</a>';
                            $post_info_html .= '</div>'; //close span.latest_post_categories
                        }
                        ...
                        if ($display_category == '1') {
                            ...
                            $post_info_html .= '<span class = "category_icon icon-tag" '.moose_elated_get_inline_style($icon_color).'></span>';
						...
						if ($display_share == '1') {
                            $post_info_html .= '<div class="blog_list_social">';
                            $post_info_html .= do_shortcode('[no_social_share]');
                            $post_info_html .= '</div>';
                        }
                        ...
                        $html .= '<li class="clearfix" ' . moose_elated_get_inline_style($minimal_style) . '>';
                        ...
                        $html .= '<div class="boxes_shader">';
                        
                        if ($display_button == '1') {
                            $html .= do_shortcode('[no_button ' . implode(' ', $button_param_array) . ' link="' . get_permalink() . '"]');
                        }

                        $html .= '</div>'; //close boxes shader

                        $html .= '</a>'; //close permalink

						if ($display_date == '1' || $display_comments == '1'){
							$html .= '<div class="boxes_corner_info">';
							if ($display_date == '1'){
								$html .= '<div class="latest_post_date_corner_holder">';
								$html .= '<span class="date_day">' . get_the_time('d') . '</span>';
								$html .= '<span class="date_month_year">' . get_the_time('M Y') . '</span>';
								$html .= '</div>';							
							}
							...
							$comments_count = get_comments_number();
							$comments_count_text = "";
							$comments_count_text .= '<span class="comments_no">' . $comments_count . '</span>';
							$comments_count_text .= '<span class="icon-bubbles comment_icon"></span>';
							$html .= '<div class="latest_post_comments_corner"> ';
							$html .= '<a class="post_comments" href="' . esc_url(get_comments_link()) . '">';
							$html .= $comments_count_text;
							$html .= '</a></div>'; //close post_comments
							}
							$html .= '</div>'; //close boxes_corner_info
						}
                        $html .= '</div>';
				...
				case 'minimal':
                        ...
                        $html .= '<div class="latest_post" ' . moose_elated_get_inline_style($box_style) . '>';
...
if (!function_exists('no_line_graph')) {
    ...
    function no_line_graph($atts, $content = null) {
        ...
        for ($i = 0; $i < count($line_graph_array); $i = $i + 1) {
            ...
            $color = moose_elated_hex2rgb(trim($line_graph_el[0]));
            $html .= "{fillColor: 'rgba(" . $color[0] . "," . $color[1] . "," . $color[2] . ",0.7)',data:[" . $values . "]}";
        }
        if (!empty($moose_elated_options['text_fontsize'])) {
            $text_fontsize = $moose_elated_options['text_fontsize'];
        } else {
            $text_fontsize = 15;
        }
        if (!empty($moose_elated_options['text_color']) && $custom_color == "") {
            $text_color = $moose_elated_options['text_color'];
        } else if (empty($moose_elated_options['text_color']) && $custom_color != "") {
...
if (!function_exists('no_message')) {
    ...
    function no_message($atts, $content = null) {
        global $moose_elated_options_theme18;
        global $moose_elated_icon_collections;
        ...
        if ($border_color != "") {
			...
			$icon_holder_styles .= "border-color: " . $border_color . ";";
        }
        ...
        if ($icon_background_color != "") {
            $icon_holder_styles .= " background-color: " . $icon_background_color . ";";
        }
        ...
        if ($type == "with_icon" || $type == "with_custom_icon") {
            $icon_html .= '<div class="eltd_message_icon_holder" style = "' . $icon_holder_styles . '"><div class="eltd_message_icon"><div class="eltd_message_icon_inner">';
			if ($custom_icon != "") {
                if (is_numeric($custom_icon)) {
                    $custom_icon_src = wp_get_attachment_url($custom_icon);
                } else {
        ...
        $html .= "<div class='message_text_holder'><div class='message_text'><div class='message_text_inner'>" . do_shortcode($content) . "</div>";
        $html .= "<a href='#' class='close'>";
        $html .= "<i class='eltd_font_elegant_icon icon_close_alt2' style='" . $close_button_style . "'></i>";
        $html .= "</a>"; //close a.close</div></div>";
		$html .= "</div></div>"; // close message text holder, close message text
...
if (!function_exists('no_pie_chart')) {
    ...
    function no_pie_chart($atts, $content = null) {
		...
		if($enable_border != '' && $enable_border == 'yes') {
			if($border_color != '') {
				$border_style = 'border-color:' . $border_color . ';';
			}
			$html .= '<span class="outer_border" style="' . $border_style . '"></span>';
			$html .= '<span class="inner_border" style="' . $border_style . '"></span>';
		}
...
if (!function_exists('no_progress_bar')) {
    ...
    function no_progress_bar($atts, $content = null) {
		...
		if($title_padding_bottom != "" && $percentage_bar_margin_bottom != "" && $percentage_bar_margin_bottom > $title_padding_bottom && $floating_type != 'floating_inside') {
			$progress_title_holder_styles .= "padding-top: " . (intval($percentage_bar_margin_bottom) - intval($title_padding_bottom)) . "px;";
		}
...
if (!function_exists('no_progress_bar_icon')) {
    ...
    function no_progress_bar_icon($atts, $content = null) {
        ...
        while ($i < $icons_number) {
            $html .= "<div class='bar' ".moose_elated_get_inline_style($custom)."><span class='bar_noactive eltd_icon_stack ";
            if ($size != "") {
                if ($size == "tiny") {
                    $html .= "eltd_tiny_icon";
                } else if ($size == "small") {
...
if (!function_exists('no_social_share')) {
    ...
    function no_social_share($atts, $content = null) {
        ...
        if (isset($moose_elated_options['twitter_via']) && !empty($moose_elated_options['twitter_via'])) {
            $twitter_via = " via " . esc_attr($moose_elated_options['twitter_via']) . " ";
        } else {
            $twitter_via = "";
        }
        ...
        if (isset($moose_elated_options['enable_social_share']) && $moose_elated_options['enable_social_share'] == "yes") {
            ...
            if (isset($moose_elated_options["post_types_names_$post_type"])) {
                if ($moose_elated_options["post_types_names_$post_type"] == $post_type) {
                    ...
                    $html .= '<a href="javascript:void(0)" target="_self"><span class="icon-share social_share_icon"></span>';
                    $html .= '<span class="social_share_title">' . esc_html__('Share', 'moose') . '</span>';
                    $html .= '</a>';
                    $html .= '<div class="social_share_dropdown"><ul>';
                    if (isset($moose_elated_options['enable_facebook_share']) && $moose_elated_options['enable_facebook_share'] == "yes") {
                        $html .= '<li class="facebook_share">';
                        if(wp_is_mobile()) {
                            $html .= '<a title="'.esc_html__("Share on Facebook",'moose').'" href="javascript:void(0)" onclick="window.open(\'http://m.facebook.com/sharer.php?u=' . urlencode(get_permalink());
                         }
                        else {
                        $html .= '<a title="'.esc_html__("Share on Facebook",'moose').'" href="javascript:void(0)" onclick="window.open(\'http://www.facebook.com/sharer.php?s=100&amp;p[title]=' . urlencode(moose_elated_addslashes(get_the_title())) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;p[images][0]=';
                        if(function_exists('the_post_thumbnail')) {
                        $html .=  wp_get_attachment_url(get_post_thumbnail_id());
                        }
                        }

                        $html .= '&amp;p[summary]=' . urlencode(moose_elated_addslashes(strip_tags(get_the_excerpt())));
                        $html .='\', \'sharer\', \'toolbar=0,status=0,width=620,height=280\');">';
                        if (!empty($moose_elated_options['facebook_icon'])) {
                            $html .= '<img src="' . esc_url($moose_elated_options["facebook_icon"]) . '" alt="" />';
                        } else {
                    ...
                    if ($moose_elated_options['enable_twitter_share'] == "yes") {
                        $html .= '<li class="twitter_share">';
                        if(wp_is_mobile()) {
                            $html .= '<a href="#" onclick="popUp=window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(moose_elated_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                            }
                            else {
                                $html .= '<a href="#" onclick="popUp=window.open(\'http://twitter.com/home?status=' . urlencode(moose_elated_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                            }
                        if (!empty($moose_elated_options['twitter_icon'])) {
                            $html .= '<img src="' . esc_url($moose_elated_options["twitter_icon"]) . '" alt="" />';
                        } else {
                    ...
                    if ($moose_elated_options['enable_google_plus'] == "yes") {
                        ...
                        if (!empty($moose_elated_options['google_plus_icon'])) {
                            $html .= '<img src="' . esc_url($moose_elated_options['google_plus_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($moose_elated_options['enable_linkedin']) && $moose_elated_options['enable_linkedin'] == "yes") {
                        ...
                        if (!empty($moose_elated_options['linkedin_icon'])) {
                            $html .= '<img src="' . esc_url($moose_elated_options['linkedin_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($moose_elated_options['enable_tumblr']) && $moose_elated_options['enable_tumblr'] == "yes") {
                        ...
                        if (!empty($moose_elated_options['tumblr_icon'])) {
                            $html .= '<img src="' . esc_url($moose_elated_options['tumblr_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($moose_elated_options['enable_pinterest']) && $moose_elated_options['enable_pinterest'] == "yes") {
                        $html .= '<li  class="pinterest_share">';
                        $image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full');
                        $html .= '<a class="share_link" href="#" onclick="popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . moose_elated_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (!empty($moose_elated_options['pinterest_icon'])) {
                            $html .= '<img src="' . esc_url($moose_elated_options['pinterest_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($moose_elated_options['enable_vk']) && $moose_elated_options['enable_vk'] == "yes") {
                        ...
                        if (!empty($moose_elated_options['vk_icon'])) {
                            $html .= '<img src="' . esc_url($moose_elated_options['vk_icon']) . '" alt="" />';
                        } else {
...
if (!function_exists('no_social_share_list')) {
    ...
    function no_social_share_list($atts, $content = null) {
        ...
        if (isset($moose_elated_options['enable_social_share']) && $moose_elated_options['enable_social_share'] == "yes") {
            ...
            $share_icons_type = '';
            if(isset($moose_elated_options['social_share_list_icons_type']) && $moose_elated_options['social_share_list_icons_type'] !== '') {
                $share_icons_type = $moose_elated_options['social_share_list_icons_type'];
            }
            ...
            if (isset($moose_elated_options["post_types_names_$post_type"])) {
                if ($moose_elated_options["post_types_names_$post_type"] == $post_type) {
                    ...
                    if ($moose_elated_options['enable_twitter_share'] == "yes") {
                        $html .= '<li class="twitter_share">';
                        if(wp_is_mobile()) {
                            $html .= '<a href="#" title="'.esc_html__("Share on Twitter", 'moose').'" onclick="popUp=window.open(\'http://twitter.com/intent/tweet?text=' . urlencode(moose_elated_the_excerpt_max_charlength(mb_strlen(get_permalink())) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                        }
                        else {
                            $html .= '<a href="#" title="'.esc_html__("Share on Twitter", 'moose').'" onclick="popUp=window.open(\'http://twitter.com/home?status=' . urlencode(moose_elated_the_excerpt_max_charlength(mb_strlen(get_permalink())) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                        }
                        if (!empty($moose_elated_options['twitter_icon'])) {
                            $html .= '<img src="' . esc_url($moose_elated_options["twitter_icon"]) . '" alt="" />';
                        } else {
                    ...
                    if ($moose_elated_options['enable_google_plus'] == "yes") {
                        $html .= '<li  class="google_share">';
                        $html .= '<a href="#" title="' . esc_html__("Share on Google+", 'moose') . '" onclick="popUp=window.open(\'https://plus.google.com/share?url=' . urlencode(get_permalink()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (!empty($moose_elated_options['google_plus_icon'])) {
                            $html .= '<img src="' . esc_url($moose_elated_options['google_plus_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($moose_elated_options['enable_linkedin']) && $moose_elated_options['enable_linkedin'] == "yes") {
                        $html .= '<li  class="linkedin_share">';
                        $html .= '<a href="#" class="' . esc_html__("Share on LinkedIn", 'moose') . '" onclick="popUp=window.open(\'http://linkedin.com/shareArticle?mini=true&amp;url=' . urlencode(get_permalink()) . '&amp;title=' . urlencode(get_the_title()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (!empty($moose_elated_options['linkedin_icon'])) {
                            $html .= '<img src="' . esc_url($moose_elated_options['linkedin_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($moose_elated_options['enable_tumblr']) && $moose_elated_options['enable_tumblr'] == "yes") {
                        $html .= '<li  class="tumblr_share">';
                        $html .= '<a href="#" title="' . esc_html__("Share on Tumblr", 'moose') . '" onclick="popUp=window.open(\'http://www.tumblr.com/share/link?url=' . urlencode(get_permalink()) . '&amp;name=' . urlencode(get_the_title()) . '&amp;description=' . urlencode(get_the_excerpt()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (!empty($moose_elated_options['tumblr_icon'])) {
                            $html .= '<img src="' . esc_url($moose_elated_options['tumblr_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($moose_elated_options['enable_pinterest']) && $moose_elated_options['enable_pinterest'] == "yes") {
                        $html .= '<li  class="pinterest_share">';
                        $image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full');
                        $html .= '<a href="#" title="' . esc_html__("Share on Pinterest", 'moose') . '" onclick="popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . moose_elated_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (!empty($moose_elated_options['pinterest_icon'])) {
                            $html .= '<img src="' . esc_url($moose_elated_options['pinterest_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($moose_elated_options['enable_vk']) && $moose_elated_options['enable_vk'] == "yes") {
                        $html .= '<li  class="vk_share">';
                        $image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full');
                        $html .= '<a href="#" title="' . esc_html__("Share on VK", 'moose') . '" onclick="popUp=window.open(\'http://vkontakte.ru/share.php?url=' . urlencode(get_permalink()) . '&amp;title=' . urlencode(get_the_title()) . '&amp;description=' . urlencode(get_the_excerpt()) . '&amp;image=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (!empty($moose_elated_options['vk_icon'])) {
                            $html .= '<img src="' . esc_url($moose_elated_options['vk_icon']) . '" alt="" />';
                        } else {
...
if (!function_exists('no_team')) {
    ...
    function no_team($atts, $content = null) {
        ...
        if ($background_color != "" || $box_border == "yes" || $box_border == "") {
        ...
        if($team_image_type != "" && $team_image_type == 'circle'){
            $eltd_team_holder_classes[] = $team_image_type;
        }
        ...
        $qteam_border_style = "";
		if ($box_border == "no") {
			$qteam_border_style .= "style='";
			$qteam_border_style .= "border: none;";
			$qteam_border_style .= "'";
		}
        if ($box_border == "yes") {
            ...
            $qteam_border_style .= "border-style:solid;";
            if ($box_border_color != "") {
                $qteam_border_style .= "border-color:" . $box_border_color . ";";
            }
            if ($box_border_width != "") {
                $qteam_border_style .= "border-width:" . $box_border_width . "px;";
            }
			$qteam_border_style .= "'";			
        }
		...
		if ($separator_width != '') {
            $eltd_team_separator_style = ' width: ' . $separator_width;
        }
		if ($separator_thickness != '') {
            $eltd_team_separator_style = ' border-bottom-width: ' . $separator_thickness;
        }
		if ($separator_margin_top != '') {
            $eltd_team_separator_style = ' margin-top: ' . $separator_margin_top;
        }
		if ($separator_margin_bottom != '') {
            $eltd_team_separator_style = ' margin-bottom: ' . $separator_margin_bottom;
        }
        ...
        if ($team_name_bottom_margin != '') {
            $eltd_team_name_style_array[] = 'margin-bottom: ' . $team_name_bottom_margin. 'px';
        }

        if ($team_name_font_size != '') {
            $eltd_team_name_style_array[] = 'font-size: ' . $team_name_font_size . 'px!important';
        }
		...
		switch ($team_type) {
			case "on_hover":
				...
				$html .= "<div class='eltd_team_inner'" . $qteam_border_style . ">";
				if ($team_image != "") {
					...
					if ($team_social_icon_pack !== '') {
						$icon_collection_obj = $moose_elated_icon_collections->getIconCollection($team_social_icon_pack);
				...
				if ($team_description != "" || $show_skills == 'yes') {
					...
					$html .= "<div class='eltd_team_text " . $text_align . "'>";
			...
			case "below_image":
				...
				if ($team_image != "") {
					$html .= "<div class='eltd_team_image'>";
                    $html .= "<div class='team_initial_image'>";
					$html .= "<img src='$team_image_src' alt='team_image'/>";
                    $html .= "</div>";
                    $html .= "<div class='team_hover_image' style='background-image: url(".$team_image_src.");'>";
                    $html .= "</div>";
					...
					if ($team_social_icon_pack !== '') {
						...
						$html .= "<div class='eltd_team_social_holder_between " .$team_hover_type. " " .$social_icons_position. "' ".  moose_elated_get_inline_style($social_image_bottom_style).">";
						$html .= "<div class='eltd_team_social ".$team_social_icon_type." '>";
						if ($team_social_style == "social_style_between") {
							$html .= '<span class="social_share_icon_shape"><span class="icon-share social_share_icon"></span></span>';
						}
						$html .= "<div class='eltd_team_social_inner'>";
						$html .= "<div class='eltd_team_social_wrapp'>";
				...
				if ($team_name !== '' || $team_position !== '' || $team_show_separator == 'yes' || $team_description != "" || $show_skills == 'yes'){
					$html .= '<div class="eltd_team_info" style="'. $qteam_box_style . '">';
...
if (!function_exists('no_service_table')) {
    ...
    function no_service_table($atts, $content = null) {
        ...
        $subtitle_color = esc_html($subtitle_color);
        ...
        if ($top_background_image != "") {
			...
			$service_icon_background_image .= 'border: 0;';
        }
		...
		if ($border_color != "") {
			$service_icon_background_image .= "border-color:" . $border_color . ";";
			$title_holder_style .= 'border-color: '.$border_color.';';
			$content_text_style .= 'border-color: '.$border_color.';';
		}        
        ...
        if ($type == 'title_on_top') {
            $html .= "<li class='service_table_title_holder " . $title_classes . "' " . moose_elated_get_inline_style($title_holder_style) . ">";
			...
			if ($subtitle != "") {
				...
				$html .= "<p class='service_subtitle' ".$subtitle_style.">" . $subtitle . "</p>";
				$html .= "</div>";
        ...
        if ($show_icon_image == 'yes') {
            ...
            if ($header_type == 'with_icon') {
                $html .= "<li class='service_icon' " . moose_elated_get_inline_style($service_icon_background_image) . " >";
        ...
        $html .= "<li class='service_table_content' " . moose_elated_get_inline_style($content_text_style) . ">";
...
if (!function_exists('no_image_slider_no_space')) {
    ...
    function no_image_slider_no_space($atts, $content = null) {
        ...
        if (isset($moose_elated_options['carousel_navigation_arrows_type']) && $moose_elated_options['carousel_navigation_arrows_type'] != '') {
            $icon_navigation_class = $moose_elated_options['carousel_navigation_arrows_type'];
        }
...
if (!function_exists('no_separator_with_icon')) {
    ...
    function no_separator_with_icon($atts, $content = null) {
		...
		if ($width != "") {
			$width = (strstr($width, 'px', true)) ? $width : $width . "px";
			$separator_lines_styles[] = "width:" . $width;
		}
		else{
			$width = '1000px';
		}
        ...
        $icon_position = '50';
        if ($type == 'with_custom_icon' && $custom_icon != '') {
            $custom_icon_dimensions_array = moose_elated_get_image_dimensions($custom_icon_src);
        ...
        $html .= '<div class="' . esc_attr($separator_classes) . '" '.moose_elated_get_inline_style($separator_styles).' data-line-width="'.esc_attr($width).'">';
        ...
        elseif($type == 'with_custom_icon'){
			$html .= '<div class="separator_custom_icon">';
			if ($link !== ''){
				$html .= '<a href="'.esc_url($link).'" target="'.esc_attr($target).'" '.moose_elated_get_class_attribute($icon_link_classes).'>';
			}
			$html .= '<img src="' . esc_url($custom_icon_src) . '" alt="" />';
			if ($link !== ''){
				$html .= '</a>';
			}
			$html .= '</div>';
            ...
            while ($blog_query->have_posts()) : $blog_query->the_post();
                ...
                switch($type) {
                    case 'info_in_bottom':
                        ...
                        $html .= '<div class="blog_text_holder blog_slider_info_in_bottom" '.moose_elated_get_inline_style($hover_box_style).'>';
						//Date and comments
                        $html .= '<div class="post_corner_info_holder">';
                        $html .= '<div class="post_date_standard_holder">';
                        $html .= '<span class="post_date_day">'. get_the_time('d') .'</span>';
                        $html .= '<span class="post_date_month_year">'. get_the_time('M') .' '. get_the_time('Y') .'</span>';
                        $html .= '</div>';
                        $html .= '<div class="post_comments_standard_holder">';
                        $html .= '<div class="post_comments_holder">';
                        $html .= '<a class="post_comments" href="'. get_comments_link() .'" target="_self">';
						$html .= '<span class="post_comments_number">' .get_comments_number() .' </span>';
                        $html .= '<i class="icon-bubbles"></i>';
                        $html .= '</a>';
                        $html .= '</div>';
                        $html .= '</div>';
                        $html .= '</div>';
                        ...
                        if ($show_date == 'yes' || $show_author=='yes' || $show_categories == 'yes') {
                            ...
                            $html.= '<div class = "blog_slider_post_info" ' . moose_elated_get_inline_style($post_info_style) . '>';

							if ($show_author == 'yes') {
								$html .= '<span class="blog_slider_author_holder"><span class="icon-user info-icon"></span><a href="' . get_author_posts_url(get_the_author_meta("ID")) . '" ' . moose_elated_get_inline_style($author_style) . '>' . get_the_author_meta("display_name") . '</a></span>';
							}

//                            if ($show_date == 'yes') {
//								$html .= '<span class="blog_slider_date_holder" ' . $date_style . '>';
//								$html .= '<span class="icon-calendar info-icon"></span>';
//								$html .= get_the_time('j M');
//								$html .= '</span>';
//							}

                            if ($show_categories == 'yes') {
                                $html .= '<div class="blog_slider_categories"><span class="icon-tag info-icon"></span>';
...
//								$html .= '<a href="' . get_comments_link() . '"><span class="icon-bubble info-icon"></span> '. get_comments_number_text() .'</a>';
//							}
                    ...
                    default:
                        ...
                        $html .= '<div class="blog_text_holder blog_slider_default ' . $info_type_class . '" ' . moose_elated_get_inline_style($hover_box_style) . '>';
                        $html .= '<div class="blog_text_holder_outer">';
                        ...
                        if ($show_separator == 'yes') {
                        	$html .= '<span class="blog_slider_separator" '. moose_elated_get_inline_style($separator_style) . '></span>';
                        }
            ...
            $html .= esc_html__('Sorry, no posts matched your criteria.', 'moose');
        endif;
        ...
        if ($blog_query->have_posts()) : $postCount = 0;
            while ($blog_query->have_posts()) : $blog_query->the_post();
				...
				if ($image_size !== 'custom' || $image_width == '' || $image_height == ''){
					...
					$html.= moose_elated_generate_thumbnail(get_post_thumbnail_id(get_the_ID()),null,$image_width,$image_height);
				}
				$html .= '</span>';
				...
				if($show_category == "yes" || $show_author == "yes" || $show_date == "yes"){
				...
				if($show_read_more == "yes"){
					...
					$html .= '<a href="'.get_the_permalink().'" target="_self" class="qbutton small read_more_button">'.esc_html__("Read More", 'moose').'</a>';
...
if (!function_exists('no_parallax_layers')) {
	...
	function no_parallax_layers($atts, $content = null) {
		...
		$html .= "<div class='eltd_parallax_layers " . $parallax_layers_holder_classes . "' style='" . $parallax_layers_holder_styles . "' ".$parallax_layers_data_styles."><div class='eltd_parallax_layers_holder preload_parallax_layers'>";
		...
		if (isset($parallax_images_array) && count($parallax_images_array) != 0) {
			...
			foreach ($parallax_images_array as $pimg_id) {
                ...
                $image_class = get_post_meta($pimg_id,'eltd_custom_class',true);

				$html .= '<div class="image '.($image_class?$image_class:'').'" style="background-image:url(' . $pimage_src . ');" ></div>';
			}
		}
...
/* Elated Magnified Image */

if (!function_exists('no_magnified_image')) {

    function no_magnified_image($atts, $content = null) {
        global $moose_elated_options;
        $args = array(
            "image" => "",
            "image_in_magnifier" => "",
        ...
        $button_style_array[] = 'style="transparent"';
        $button_style_array[] = 'button_hover_animation="default"';
        $button_style_array[] = 'icon_pack="simple_line_icons"';
        $button_style_array[] = 'simple_line_icons="icon-rocket"';
        ...
        $subtitle_style_array[] = 'text_in_box="no"';
        $subtitle_style_array[] = 'line_width="40"';
        $subtitle_style_array[] = 'box_padding="10"';

        if($subtitle !== "") {
            $subtitle_style_array[] = 'title="' . $subtitle . '"';
        }

        $subtitle_style = implode(' ', $subtitle_style_array);

        $html .= "<div class='eltd_magnified_image_holder'>";
        ...
        if($image_in_magnifier !== "") {
            $html .= "<div class='magnifier_holder'>";
            if (is_numeric($image_in_magnifier)) {
                $image_src = wp_get_attachment_url($image_in_magnifier);
            } else {
                $image_src = $image_in_magnifier;
            }
            $html .= '<img src="' . $image_src . '" alt="' . $title . '" />';
            $html .= "</div>";
        }
        $html .= "<div class='overlay_holder_wrapper'>";
            $html .= "<div class='overlay_holder'>";
            if($subtitle !== "") {
                $html .= "<div class='subtitle_hodler'>" . do_shortcode('[vc_text_separator ' . $subtitle_style . ']') . "</div>";
            }
            if($title !== "") {
                $html .= "<div class='title_hodler'><$title_tag>" . $title . "</$title_tag></div>";
            }
            if($text !== "") {
                $html .= "<div class='text_hodler'><p>" . $text . "</p></div>";
            }
    ...
    add_shortcode('no_magnified_image', 'no_magnified_image');
}


if (!function_exists('no_image_stack')) {

    function no_image_stack($atts, $content = null) {
        $args = array(
            "height"  => "895",
            "image_1" => "",
            "image_2" => "",
            ...
            "pop_up_image" => "",
            "pop_up_image_link" => "",
            "pop_up_image_shader_color" => "#fff",
            "responsive_overlay_color" => "rgba(0,0,0,0.7)"
        );
        ...
        if ($height !='0') {
            ...
            $html .='<div class="image_stack" style="height:'.$height.'px;">';
            $html .='<div class="slide_out_image_holder">';

                //image 4
                if (is_numeric($image_4)) {
                    $image_4_src = wp_get_attachment_url($image_4);
                } else {
                    $image_4_src = $image_4;
                }

                $html .= '<div class="image image_1of5 outer" style="background-image: url('.$image_4_src.');"></div>';


                // image 2
                if (is_numeric($image_2)) {
                    $image_2_src = wp_get_attachment_url($image_2);
                } else {
                    $image_2_src = $image_2;
                }
                
                $html .= '<div class="image image_2of5 inner" style="background-image: url('.$image_2_src.');"></div>';

                // image 1
                if (is_numeric($image_1)) {
                    $image_1_src = wp_get_attachment_url($image_1);
                } else {
                    $image_1_src = $image_1;
                }
                
                $html .= '<div class="image image_3of5" style="background-image: url('.$image_1_src.');"></div>';

                $html .= '<div class="responsive_overlay" style="background-color:'.$responsive_overlay_color.';"></div>';

                // image 3
                if (is_numeric($image_3)) {
                    $image_3_src = wp_get_attachment_url($image_3);
                } else {
                    $image_3_src = $image_3;
                }
                
                $html .= '<div class="image image_4of5 inner" style="background-image: url('.$image_3_src.');"></div>';

                //image 5
                if (is_numeric($image_1)) {
                    $image_5_src = wp_get_attachment_url($image_5);
                } else {
                    $image_5_src = $image_5;
                }

                $html .= '<div class="image image_5of5 outer" style="background-image: url('.$image_5_src.');"></div>';


             $html .='</div>'; //close slide out image holder 

             if ($pop_up_image != '') {

                 $html .='<div class="pop_up_image_holder">';

                    if ($pop_up_image_link != '') {

                        $html .= '<a href="' . $pop_up_image_link . '"></a>';

                    }

                    //pop_up image
                    if (is_numeric($pop_up_image)) {
                        $pop_up_image_src = wp_get_attachment_url($pop_up_image);
                    } else {
                        $pop_up_image_src = $pop_up_image;
                    }

                    $html .= '<div class="pop_up_image" style="background-image: url('.$pop_up_image_src.');"></div>';
                    $html .= '<div class="shader" style="background-color:'.$pop_up_image_shader_color.';"></div>';

                $html .='</div>'; //close pop_up_image holder
            } 
            $html .='</div>'; //close image stack
    ...
    add_shortcode('no_image_stack', 'no_image_stack');
