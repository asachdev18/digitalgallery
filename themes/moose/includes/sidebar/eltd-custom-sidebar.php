...
if( ! class_exists('MooseElatedSidebar') ){
	
	class MooseElatedSidebar{
		...
		function __construct(){
			...
			$this->title = esc_html__('Custom Widget Area','moose');
		...
		function load_assets(){
			...
			wp_enqueue_script('eltd_sidebar' , MOOSE_ELATED_ROOT . '/framework/admin/assets/js/eltdf-sidebar.js');
			wp_enqueue_style( 'eltd_sidebar' , MOOSE_ELATED_ROOT . '/framework/admin/assets/css/eltdf-sidebar.css');
		}
		...
		function template_add_widget_field(){
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".esc_html__('Enter Name of the new Widget Area', 'moose')."' name='eltd-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".esc_html__('Add Widget Area', 'moose')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
