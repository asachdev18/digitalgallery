...
$args = array(
	...
	'side_padding' => '',
	'text_align' => 'left',
	'more_button_label' => esc_html__('Expand Section', 'moose'),
	'less_button_label'=> esc_html__('Contract Section', 'moose'),
...
$args = array_merge($args, $moose_elated_icon_collections->getShortcodeParams('content_menu_'));
...
if ($icon_pack !== '' && $in_content_menu !== '') {
    ...
    $item_collection_icon_obj = $moose_elated_icon_collections->getIconCollection($icon_pack);
...
if($row_type == 'row') {
	...
	if ($enable_badge == "yes"){
		...
		$output .= '<span class="badge_icon"><img src="'.get_template_directory_uri().'/img/crown.png" alt="'. esc_html__('Crown', 'moose') .'"/></span>';
		if ($badge_text !== ''){
	...
	if($video == "show_video"){
        ...
        if($use_row_as_full_screen_section == "yes"){
            ...
            $output .='<object width="320" height="240" type="application/x-shockwave-flash" data="'.get_template_directory_uri().'/js/flashmediaelement.swf">
										...
										<img src="'.$v_image.'" width="1920" height="800" title="' . esc_html__('No video playback capabilities', 'moose') . '" alt="' . esc_html__('Video thumb', 'moose') . '" />
								</object>
						</video>';
        } else {
	...
	if($logo_in_content_menu == 'logo_in_content_menu'){
		$output .= '<div class="logo">';
			...
			if(isset($moose_elated_options['content_menu_logo_image']) && $moose_elated_options['content_menu_logo_image'] != ""){
				$logo_image = $moose_elated_options['content_menu_logo_image'];
			}else if(isset($moose_elated_options['logo_image']) && $moose_elated_options['logo_image'] != ""){
				$logo_image = $moose_elated_options['logo_image'];
			}
			$output .= '<a href="'.esc_url(home_url('/')).'"><img src="'.esc_url($logo_image).'" alt="' . esc_attr__( 'Logo', 'moose' ) . '" /></a>';
		$output .= '</div>';
	}
	...
	if($custom_widget_area !== ''){
		$output .= '<div class="custom_widget_area"><div class="custom_widget_area_inner"><div class="custom_widget_area_inner2">';
			$output .= moose_elated_get_dynamic_sidebar($custom_widget_area);
		$output .= '</div></div></div>';
	}
...
echo moose_elated_get_module_part($output);