...
if (isset($moose_elated_options['blog_standard_type_show_categories'])){
	$blog_show_categories = $moose_elated_options['blog_standard_type_show_categories'];
}

$blog_show_comments = "yes";
if (isset($moose_elated_options['blog_standard_type_show_comments'])){
    $blog_show_comments = $moose_elated_options['blog_standard_type_show_comments'];
}
$blog_show_author = "yes";
if (isset($moose_elated_options['blog_standard_type_show_author'])){
    $blog_show_author = $moose_elated_options['blog_standard_type_show_author'];
}
$blog_show_like = "yes";
if (isset($moose_elated_options['blog_standard_type_show_like'])) {
    $blog_show_like = $moose_elated_options['blog_standard_type_show_like'];
}
$blog_show_ql_icon_mark = "yes";
$blog_title_holder_icon_class = "";
if (isset($moose_elated_options['blog_standard_type_show_ql_mark'])) {
    $blog_show_ql_icon_mark = $moose_elated_options['blog_standard_type_show_ql_mark'];
}
...
$blog_show_date = "yes";
if (isset($moose_elated_options['blog_standard_type_show_date'])) {
    $blog_show_date = $moose_elated_options['blog_standard_type_show_date'];
}
$blog_show_social_share = "no";
$blog_social_share_type = "dropdown";
if(isset($moose_elated_options['blog_standard_type_select_share_option'])){
	$blog_social_share_type = $moose_elated_options['blog_standard_type_select_share_option'];
}
if (isset($moose_elated_options['enable_social_share'])&& ($moose_elated_options['enable_social_share']) =="yes"){
    if (isset($moose_elated_options['post_types_names_post'])&& $moose_elated_options['post_types_names_post'] =="post"){
        if (isset($moose_elated_options['blog_standard_type_show_share'])&& $blog_social_share_type == "dropdown") {
            $blog_show_social_share = $moose_elated_options['blog_standard_type_show_share'];
        }
    }
}

$blog_ql_background_image = "yes";
if(isset($moose_elated_options['blog_standard_type_ql_background_image'])){
	$blog_ql_background_image = $moose_elated_options['blog_standard_type_ql_background_image'];
}
...
if (isset($moose_elated_options['blog_standard_type_read_more_button_icon']) && $moose_elated_options['blog_standard_type_read_more_button_icon'] == 'yes'){
    $blog_read_more_button_classes .= 'with_icon';
}


$blog_image_size = 'full';
if( isset($moose_elated_options['blog_standard_type_image_size']) && $moose_elated_options['blog_standard_type_image_size'] !== '') {
    $blog_image_size = 'eltd-moose-'.$moose_elated_options['blog_standard_type_image_size'];

}

if( $blog_image_size == 'custom'
    && isset($moose_elated_options['blog_standard_type_image_size_height']) && $moose_elated_options['blog_standard_type_image_size_height'] !== ''
    && isset($moose_elated_options['blog_standard_type_image_size_width']) && $moose_elated_options['blog_standard_type_image_size_width'] !== '') {

    $image_height = $moose_elated_options['blog_standard_type_image_size_height'];
    $image_width = $moose_elated_options['blog_standard_type_image_size_width'];

    $image_html = moose_elated_generate_thumbnail(get_post_thumbnail_id(get_the_ID()),null,$image_width,$image_height);
}
else{
					    	...
					    	<div class="video-wrap"  >
							    <video class="video" poster="<?php echo esc_url(get_post_meta(get_the_ID(), "video_format_image", true));  ?>" preload="auto">
								    ...
								    <object width="320" height="240" type="application/x-shockwave-flash" data="<?php echo esc_url(get_template_directory_uri().'/js/flashmediaelement.swf'); ?>">
									    ...
									    <img src="<?php echo esc_url(get_post_meta(get_the_ID(), "video_format_image", true));  ?>" width="1920" height="800" title="<?php esc_attr_e( 'No video playback capabilities', 'moose' ); ?>" alt="<?php esc_attr_e( 'Video thumb', 'moose' ); ?>" />
								    </object>
							    </video>
						    </div>
                        </div>
						...
						<div class="post_comments_standard_holder">
							<div class="post_comments_holder">
								<a class="post_comments" href="<?php comments_link(); ?>" target="_self">
									...
									<span class="post_comments_text"><?php comments_number(esc_html__('comments','moose'),esc_html__('comment','moose'),esc_html__('comments','moose') ); ?></span>
								</a>
							</div>
						</div>
						<?php }
                 ...
                 <div class="post_text">
                    <div class="post_text_inner">         
						...
						<?php if($blog_show_author == "yes" || $blog_show_social_share == "yes" || $blog_show_categories == "yes" || $blog_show_like == "yes") { ?>	
							<div class="post_info">
								<?php moose_elated_post_info(array('author' => $blog_show_author)); ?>
								<?php moose_elated_post_info(array('category' => $blog_show_categories)); ?>
								<?php moose_elated_post_info(array('share' => $blog_show_social_share)); ?>
								<?php moose_elated_post_info(array('like' => $blog_show_like)); ?>
							</div>
						<?php } ?>
						<?php
							the_content();
							moose_elated_read_more_button('blog_standard_type_read_more_button',$blog_read_more_button_classes);
						?>
						<?php if(isset($moose_elated_options['blog_standard_type_show_share']) && $moose_elated_options['blog_standard_type_show_share'] == "yes" && $blog_social_share_type == "list") {
			...
			<div class="post_content_holder">
				...
				<div class="audio_image">
					<audio class="blog_audio" src="<?php echo esc_url(get_post_meta(get_the_ID(), "audio_link", true)) ?>" controls="controls">
						<?php esc_html_e("Your browser don't support audio player",'moose'); ?>
					</audio>
				</div>
                <div class="post_text">
                    <div class="post_text_inner">
